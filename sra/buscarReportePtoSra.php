...
<!DOCTYPE html>
<html>
<?php echo $c_funciones->getHeaderNivel2("Buscar Reporte SRA Punto Evaluación", 
	'  <style>
  .panel-content {
    padding: 1em;
  }
						...
						<table data-role="table" id="movie-table" data-filter="true" data-input="#filterTable-input" class="ui-responsive">
						    <thead>
						        <tr>
						            ...
						            <th data-priority="2">Nombre Punto</th>
...
<?php
								$result = $c_funciones->getReportesSraPtos();
									while ($row = mysqli_fetch_array($result, MYSQL_NUM)){
										...
										echo '<th>'.$row[0].'</th>';
										echo '<td>'.$row[1].'</td>';
										echo '<td><a href=mostrarReporteSra.php?idReporte='.$row[0].' data-ajax="false">'.$row[2].'</a></td>';
										echo '<td>'.$row[3].'</td>';										
										echo '</tr>';			
									}
?>	
